...
class DashboardRemoteService {
  DashboardRemoteService({
    ...
    this.path = '/api/bp/GetDashboardCountv1',
    ...
    try {
      ...
      if (decoded is! Map<String, dynamic>) {
        throw const DashboardFailure(
          ...
          message: 'Invalid dashboard response format.',
        );
      }

      final envelope = DashboardResponse.fromJson(decoded);
      return envelope.parseInnerSummary();
    } on DashboardFailure {
      rethrow;
    } on TimeoutException {
      throw const DashboardFailure(
      ...
      throw const DashboardFailure(
        type: DashboardFailureType.invalidJson,
        message: 'Invalid dashboard JSON response.',
      );
    } catch (e) {
