...
/// Fetches Projects-only businesses from `GetBusiness`.
class ProjectBusinessRemoteService {
  ProjectBusinessRemoteService({
    http.Client? client,
    this.baseUrl = LocalStrings.bpms,
    this.path = '/api/bp//GetBusiness',
  ...
  Future<List<ProjectBusiness>> fetchBusinesses() async {
    ...
    try {
      ...
      if (response.statusCode == 403) {
        throw const DashboardFailure(
          ...
          message: 'You do not have permission to load businesses.',
        );
      }
      ...
      if (decoded is! Map<String, dynamic>) {
        throw const DashboardFailure(
          ...
          message: 'Invalid GetBusiness response format.',
        );
      }

      final envelope = ProjectBusinessResponse.fromJson(decoded);
      return envelope.data
          .where((b) => b.businessId > 0 && b.businessName.isNotEmpty)
      ...
      throw const DashboardFailure(
        type: DashboardFailureType.invalidJson,
        message: 'Invalid GetBusiness JSON response.',
      );
    } catch (e) {
