...
class VisualChartsRemoteService {
  VisualChartsRemoteService({
    http.Client? client,
    this.baseUrl = LocalStrings.bpms,
    this.path = '/api/bp/GetReportsForDashboard',
  ...
  Future<List<VisualChartItem>> fetchCharts({
    required String userType,
    required int userId,
  }) async {
    final uri = Uri.parse('$baseUrl$path');
    final body = jsonEncode({
      'user_type': userType =='MAN' || userType =='BH' ? 'ZM' : userType,
      'user_id': userId,
    });
    ...
    try {
      ...
      if (response.statusCode == 403) {
        throw const DashboardFailure(
          ...
          message: 'You do not have permission to view visual charts.',
        );
      }
      ...
      if (decoded is! Map<String, dynamic>) {
        throw const DashboardFailure(
          ...
          message: 'Invalid visual charts response.',
        );
      }

      final envelope = VisualChartsResponse.fromJson(decoded);
      if (envelope.success != 200 && envelope.data.isEmpty) {
        throw DashboardFailure(
          type: DashboardFailureType.unknown,
          message:
              'Unable to load visual charts (success=${envelope.success}).',
        );
      }
      ...
      throw const DashboardFailure(
        type: DashboardFailureType.invalidJson,
        message: 'Invalid visual charts JSON response.',
      );
    } catch (e) {
