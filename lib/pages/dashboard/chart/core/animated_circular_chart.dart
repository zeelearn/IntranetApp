...
class AnimatedCircularChartState extends State<AnimatedCircularChart>
  ...
  final TextPainter _labelSubPainter = new TextPainter();
  ...
  void didUpdateWidget(AnimatedCircularChart oldWidget) {
    ...
    if (oldWidget.holeSubLabel != widget.holeSubLabel ||
        oldWidget.labelStyle != widget.labelStyle) {
      _updateSubLabelPainter();
    }
  }
  ...
  void _updateLabelPainter() {
    if (widget.holeLabel != null) {
      //TextStyle _labelStyle = LightColors.headerTitleSelected;
      final TextStyle customStyle = GoogleFonts.poppins(fontSize: 32,color: LightColors.kDarkBlue, fontWeight: FontWeight.normal);
      _labelPainter
        ..text = new TextSpan(style: customStyle, text: widget.holeLabel)
  ...
  void _updateSubLabelPainter() {
    if (widget.holeLabel != null) {
      TextStyle _labelStyle = LightColors.subTextStyle;
      final TextStyle customStyle = GoogleFonts.poppins(fontSize: 18,color: LightColors.kGreen, fontWeight: FontWeight.normal );
      _labelSubPainter
        ..text = new TextSpan(style: customStyle, text: widget.holeSubLabel)
        ..textDirection = Directionality.of(context)
        ..textScaleFactor = MediaQuery.of(context).textScaleFactor
        ..layout(minWidth: 0, maxWidth: double.infinity);
    } else {
      _labelSubPainter.text = null;
    }
  }
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
            ...
            painter: new AnimatedCircularChartPainter(
              ...
              widget.holeSubLabel != null ? _labelSubPainter : null,
            ),
          );
  }
}